...
// ✅ Tambahkan import font Inter dari Next.js Google Fonts
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        inter.className, // ✅ gunakan font Inter di seluruh navbar
        className
      )}
    >
...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      // ✅ Ubah warna menjadi putih permanen
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-white px-4 py-2 lg:flex shadow-sm",
        className
      )}
    >
      {children}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-white px-4 py-8 shadow-sm",
            className
          )}
        >
          {children}
...
// ✅ Tambahkan shadow tipis pada button
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-4 py-2 rounded-md bg-white  text-black text-sm font-semibold relative cursor-pointer hover:-translate-y-0.5 transition duration-200 inline-block text-center shadow-sm";

  const variantStyles = {
    primary: "",
    secondary: "bg-transparent shadow-none",
    dark: "bg-black text-white shadow-sm",
    gradient: "bg-gradient-to-b from-blue-500 to-blue-700 text-white shadow-sm",
  };
